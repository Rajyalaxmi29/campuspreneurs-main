...
export function Navbar() {
  ...
  const getNavLinks = () => {
    ...
    // Show departments to any admin (superadmin role doesn't exist in schema)
    if (isAdmin) {
      baseLinks.push({ name: "Departments", path: "/admin/departments" });
      baseLinks.push({ name: "Dashboard", path: "/admin" });
    } else {
      baseLinks.push({ name: "Registration", path: "/registration" });
    }
