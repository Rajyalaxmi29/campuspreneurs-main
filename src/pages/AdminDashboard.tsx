...
export default function AdminDashboard() {
  ...
  const handleProblemFilterChange = (problemId: string) => {
  ...
  const handleProblemStatsFilterChange = (problemId: string) => {
    setProblemStatsFilter(problemId);
  };
  ...
  const handleDeleteAllByProblem = async (problemId: string) => {
    try {
      ...
      const { error } = await (supabase as any).from("team_registrations").delete().eq("problem_id", problemId);
  ...
  const handleSaveTeam = async (teamData: Omit<TeamRegistration, "id" | "created_at">) => {
    try {
      if (selectedTeam) {
        ...
        const updatedTeams = teamRegistrations.map(team =>
          team.id === selectedTeam.id
            ? {
                ...team,
                ...teamData,
                problem_title: problems.find(p => p.problem_statement_id === teamData.problem_id)?.title || "Unknown Problem",
                theme: problems.find(p => p.problem_statement_id === teamData.problem_id)?.theme || "Unknown Theme"
              }
            : team
        );
        setTeamRegistrations(updatedTeams);
  ...
  const confirmDeleteAllTeams = async () => {
    const problemId = problemFilter; // Assuming you want to delete by the selected filter
    if (problemId === "all") {
      console.error("Please select a problem to delete all teams for.");
      return;
    }
    await handleDeleteAllByProblem(problemId);
  };
  ...
  const problemChartData = problemStats
    .sort((a, b) => b.count - a.count);

  const themeChartData = themeStats.filter(t => t.count > 0);
                ...
                <AccordionContent className="px-6 pb-4">
                  {problemStats.length > 0 ? (
                    <>
                      {/* Filter Control */}
                      <div className="mb-6">
                        <label htmlFor="problem-stats-filter" className="block text-sm font-medium text-foreground mb-2">
                          Filter by Problem Statement
                        </label>
                        <select
                          id="problem-stats-filter"
                      ...
                      <div className="mb-6 space-y-4">
                        <div className="flex flex-wrap gap-4">
                          <div className="flex-1 min-w-[200px]">
                            <label htmlFor="problem-filter" className="block text-sm font-medium text-foreground mb-2">
                              Filter by Problem Statement
                            </label>
                            <select
                              id="problem-filter"
                          ...
                          <div className="flex-1 min-w-[200px]">
                            <label htmlFor="theme-filter" className="block text-sm font-medium text-foreground mb-2">
                              Filter by Theme
                            </label>
                            <select
                              id="theme-filter"
                          ...
                          <div className="flex-1 min-w-[200px]">
                            <label htmlFor="sort-field" className="block text-sm font-medium text-foreground mb-2">
                              Sort by
                            </label>
                            <select
                              id="sort-field"
